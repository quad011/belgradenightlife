...
function fillTx(el) {
  ...
  var default_ttl = 'BOOK NOW';

  var par = $(el).prev();
  var ttl = par.children('.order-title').text();
  var sub = par.children('.order-sub').text();

  var mssgTxt = 'Hi, I would like to book a ' + ttl + ' - ' + sub;
  $('#message').val(mssgTxt);


  var main_par = $(el).prev().parent();

  // prevent
  $('.order-button').removeAttr('style');
  $('.order-button').removeAttr('disabled');
  $('.order-button').children('span').text(default_ttl);
  $('.order-button').prev().parent().removeAttr('style');

  main_par.css('background-color', '#ffca28');
  $(el).attr('disabled', 'disabled');
  $(el).children('span').text('RESERVED');
}
